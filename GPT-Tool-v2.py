...
def add_account_interactive(cfg):
    ...
    try:
        ...
        return
    uname, uid = read_user_from_prefs(pkg)
    if not uid:
        print("[!] Could not read UserID from prefs. You can type UserID manually.")
...
def auto_scan_packages(cfg):
    ...
    accounts = load_accounts()
    for p in pkgs:
        uname, uid = read_user_from_prefs(p)
        if not uid:
...
def main_menu():
    ...
    while True:
        ...
        print("+{:-^78}+".format(""))
        print()
        print("1) Start Auto Rejoin (install lua & monitor)")
        print("2) Install/Update Lua autoexec only")
        print("3) Add account (select package)")
        print("4) Auto-scan packages by prefix")
